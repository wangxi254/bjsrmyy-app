<template>
	<view class="detailPage pageContainer">
		...
		<hs-card class="appointdate-view">
			...
			<view>时间：<text class="textRed">{{appointmentInfo.currentDate}}{{' ' + appointmentInfo.reg_time}}</text>
			</view>
	...
	export default {
		...
		methods: {
			getNotice() {
				...
				console.log('传递的时间 = ', dateStr)
				console.log('获取到的时间 = ', time)
				if (time === dateStr) {
					return
				}
				this.$request({
			...
			dialogConfirm() {
				...
				let query = {
					...
					medicalRecordNo: this.userInfo.mrn && this.userInfo.mrn.length > 0 ? this.userInfo.mrn : this
						.PaientCard.mrn,
